...
int 	julia_util(t_env *env, double x, double y)
{
	...
	i = 0;
	x = 2 * (x - env->half_width) / (env->half_width) * env->jul->scale + env->jul->x_off;
	y = 2 * (y - env->half_height) / (env->half_height) * env->jul->scale + env->jul->y_off;
	xx = x * x;
	yy = y * y;
...
void	julia(t_env *env, int x, int y)
{
	...
	while (ir < ir_max)
	{
		...
		while (ic < ic_max)
		{
			...
			if (i < env->jul->max)
				pixel_to_img(env->jul->img, ic, ir, julia_map[i]);
			else
